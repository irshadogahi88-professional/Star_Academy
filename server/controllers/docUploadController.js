...
const extractMCQsFromText = async (rawText, subject = 'Physics', classLevel = 'XI', filename = 'Document_Import', fileBuffer = null, mimeType = null) => {
  ...
  if (apiKey && apiKey !== 'your_gemini_api_key_here') {
    try {
      ...
      const prompt = `You are an expert educational AI content extractor specialized in parsing MDCAT, ECAT, Physics, Chemistry, Biology, Mathematics, and Logical Reasoning (LR) exam papers. Extract all Multiple Choice Questions (MCQs) from this document for the subject "${subject}" (Level/Class: "${classLevel}").

      CRITICAL SYSTEM RULES:
      1. IGNORE HEADERS, FOOTERS & ACADEMY METADATA: Absolutely ignore document titles, school/academy names ("Star Academy Ghotki"), teacher/faculty names ("Sir Irshad Ogahi"), student metadata (Name, Roll No), test instructions, total marks, time limit, page numbers, or general headings. Do NOT treat these as questions or options.
      2. IDENTIFY REAL QUESTIONS: Every MCQ has a clear question statement and exactly 4 options. A question usually starts with a number (e.g. "1.", "2.", "Q1.", etc.).
      3. PRESERVE ALL MATHEMATICAL AND SCIENTIFIC SYMBOLS: Keep all formulas, Greek letters, math symbols, and LaTeX formatting exactly as they are in the text (e.g., "sin θ", "1 --> ∅ ∝", "\\frac{v}{2l}", "\\frac{\\mathrm{v}}{4\\mathrm{l}}", "\\frac{3\\mathrm{v}}{4\\mathrm{l}}", "\\frac{4\\mathrm{v}}{\\mathrm{l}}"). Do NOT strip backslashes or corrupt the LaTeX/Unicode notation.
      4. CLEAN OPTIONS: Extract exactly 4 options. Strip the option letter prefixes (like "A)", "B)", "a.", "b.", etc.) from the option text.
      5. CORRECT INDEX: Identify the correct answer from the text (marked via bold text, underline, asterisk *, or an answer key list like "Ans: C", "Correct: B"). Set correctIndex to the 0-based index of that choice (0 = A, 1 = B, 2 = C, 3 = D). If the document does not specify the correct answer, solve the question scientifically/mathematically and select the correct index. Do NOT default to 0 for all questions.
      6. EXPLANATION: If the document provides a solution or explanation, extract it. Otherwise, generate a brief, clear, and scientifically correct explanation detailing why the selected option is correct.`;
